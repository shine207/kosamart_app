...
var app = {
    ...
    onDeviceReady: function() {
        ...
        async.waterfall([
            ...
            function(cb){
              // android filesystem init..
                if (device.platform.toLowerCase() == "android") {
                  window.requestFileSystem(LocalFileSystem.TEMPORARY, 0, (function(){cb();}), (function(err){cb(err);}));
                } else{
                  cb();
                }
            },
                        ...
                        if ( err ) {
                            ...
                            that.modifyAddress( result.address, new_address, function(err) {
                                if ( err )
                                    ...
                                    fs.writeFile(ADDRESSBOOK_INFO, result, function(err) {
                                        cb(err);
                                    });
                                }
                            });
                ...
                if ( contacts[i]['phoneNumbers'] )
                    for ( var j = 0; j<contacts[i]['phoneNumbers'].length; j++ ) {
                        ...
                        if ( phone_regex.test(phonenumber) === true )
                            result.push( phonenumber.replace(/-/g,'') );
                    }
            }
            cb(null, result);
        }
